...
teleport =
"The Ondo Cups"       --Enter the name of the Teleporter where youu farm Fates so it teleport back to the area and keeps farming
...
fatewait = 2                --the amount how long it should when before dismounting (0 = at the edge of the fate 3-5 = should be in the middle of the fate)
...
FatesBlacklist = { --Fates to blacklist
    647,           -- Gorgimera, too difficult solo
    1443,          -- Pluck of the draw, collect fate
    1453,          -- Picking up the pieces, collect fate
    1459,          -- Monkeying Around, collect fate
    ...
    if GetCharacterCondition(45) == false then
        ...
        for i = 0, fates.Count - 1 do
            ...
            if GetFateDuration(fates[i]) > 0 and GetFateProgress(fates[i]) < CompletionToIgnoreFate and IsBlackListed(tempfate) == false then
                distance = GetDistanceToPoint(GetFateLocationX(fates[i]), GetFateLocationY(fates[i]),
                    GetFateLocationZ(fates[i]))
